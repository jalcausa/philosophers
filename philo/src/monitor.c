...
/*   Updated: 2025/06/14 13:53:46 by jalcausa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_philosopher_death(t_table *table, int i)
{
    ...
    if (current_time - last_meal > table->time_to_die)
    {
        ...
        if (!table->simulation_ended)
        {
            ...
            print_death(&table->philosophers[i]);  // ← Usar la nueva función
            return (1);
        }
        pthread_mutex_unlock(&table->death_mutex);
    }
...
int	check_all_ate_enough(t_table *table)
{
    ...
    // PRIMERO verificar muertes para TODOS los casos
    i = 0;
    ...
    // LUEGO verificar si comieron suficiente (solo si hay límite)
